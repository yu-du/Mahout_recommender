...
/* 
A revised version of GenericItemBasedRecommender class in Mahout. The original version is hosted at 
http://grepcode.com/file/repo1.maven.org/maven2/org.apache.mahout/mahout-core/0.8/org/apache/mahout/cf/taste/impl/recommender/GenericItemBasedRecommender.java
Code snippets I've inserted are enclosed or followed by comments using //<Yu Du's code> and //</Yu Du's code>.
*/
...
public class GenericItemBasedRecommender extends AbstractRecommender implements ItemBasedRecommender {
  ...
  protected float doEstimatePreference(long userID, PreferenceArray preferencesFromUser, long itemID)
    ...
    System.out.printf("doEstimatePref(u%d,i%d):",userID,itemID);//<Yu Du's code>
    for (int i = 0; i < similarities.length; i++) {
      double theSimilarity = similarities[i];
      if (!Double.isNaN(theSimilarity)) {
        // Weights can be negative!
        preference += theSimilarity * preferencesFromUser.getValue(i);
        System.out.printf("+%.2f *%.0f",theSimilarity,preferencesFromUser.getValue(i));
        totalSimilarity += theSimilarity;
        count++;
      }
    }
    
    System.out.printf("=%.2f /%.2f ",preference, totalSimilarity); //<Yu Du's code>
    ...
    System.out.printf("  SimilarityCount=%d%n ",  count); 
    //</Yu Du's code>
